...
class LrSpider(Spider):
    ...
    def parse(self, response):
        ...
        for i in range(0, len(sites), 2):
            ...
            if len(sites[i].xpath('div/div/span/text()').extract()[0].encode('ascii','ignore')) > 0:
                item['vote_up'] = int(sites[i].xpath('div/div/span/text()').extract()[0])
            if len(sites[i].xpath('div/div/span/text()').extract()[1].encode('ascii','ignore')) > 0:
                item['vote_down'] = int(sites[i].xpath('div/div/span/text()').extract()[1])
